...
public class HealerWeaponFactory {
    Equipment createSanctumIcon() {
        return new Equipment("Sanctum Icon", "Weapon", "Healer", 5, 3, 0, 2, 0, 0, 0,
                "A sacred emblem radiating calm, channeling restorative energy to mend wounds with gentle precision.",
                EquipmentRarity.FORGED);
    }

    Equipment createLifebloomMedallion() {
        return new Equipment("Lifebloom Medallion", "Weapon", "Healer", 8, 4, 0, 3, 0, 0, 1,
                "Infused with the essence of life, it subtly accelerates healing for allies within its soothing aura.",
                EquipmentRarity.FORGED);
    }

    Equipment createCelestineGlobe() {
        return new Equipment("Celestine Globe", "Weapon", "Healer", 12, 6, 0, 4, 0, 0, 2,
                "Glowing with celestial light, it nurtures vitality and rejuvenates even the weariest of hearts.",
                EquipmentRarity.FORGED);
    }

    Equipment createVerdantCrucible() {
        return new Equipment("Verdant Crucible", "Weapon", "Healer", 15, 8, 0, 5, 0, 0, 2,
                "A verdant vessel of nature’s power, empowering the healer to mend grievous injuries swiftly.",
                EquipmentRarity.ENCHANTED);
    }

    Equipment createMoonlitTalisman() {
        return new Equipment("Moonlit Talisman", "Weapon", "Healer", 19, 10, 0, 5, 0, 0, 4,
                "Batched in silver light, its charm enhances the strength of restorative magic under any night sky.",
                EquipmentRarity.ENCHANTED);
    }

    Equipment createAuroraSphere() {
        return new Equipment("Aurora Sphere", "Weapon", "Healer", 20, 9, 0, 5, 0, 0, 6,
                "Shimmering with prismatic hues, each pulse restores vigor to allies, weaving a veil of protection.",
                EquipmentRarity.ENCHANTED);
    }

    Equipment createEvergloamSigil() {
        return new Equipment("Evergloam Sigil", "Weapon", "Healer", 23, 12, 0, 6, 0, 0, 5,
                "A mystical sigil resonating with twilight energy, it bolsters the healer’s command over life’s flow.",
                EquipmentRarity.MYSTIC);
    }

    Equipment createRadiantPendant() {
        return new Equipment("Radiant Pendant", "Weapon", "Healer", 26, 14, 0, 6, 0, 0, 6,
                "Encased in radiant crystal, it channels pure restorative force, sustaining allies in the heat of battle.",
                EquipmentRarity.MYSTIC);
    }

    Equipment createEthereonSanctifier() {
        return new Equipment("Ethereon Sanctifier", "Weapon", "Healer", 29, 16, 0, 7, 0, 0, 6,
    ...
    Equipment createSoulweaverAegis() {
        return new Equipment("Soulweaver Aegis", "Weapon", "Healer", 30, 7, 0, 15, 0, 0, 8,
                "Threads of souls entwine within, subtly mending and empowering, binding life’s fragility to the healer’s will.",
                EquipmentRarity.ETHEREAL);
    }

    Equipment createLuminalNexus() {
        return new Equipment("Luminal Nexus", "Weapon", "Healer", 30, 20, 0, 5, 0, 0, 5,
                "A nexus of pure light, it radiates restorative energy, guiding allies from the brink with unwavering grace.",
                EquipmentRarity.ETHEREAL);
    }

    Equipment createAscHalienRod() {
        return new Equipment("Asc Halien Rod", "Weapon", "Healer", 35, 15, 0, 10, 0, 0, 10,
