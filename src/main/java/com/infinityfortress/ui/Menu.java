...
class BattleMenu implements Menu {
    ...
    @Override
    public void display(ArrayList<NCharacter> player, ArrayList<NCharacter> enemy, ArrayList<String> turnOrder,
            int choice) {
        ...
        for (int i = 0; i < 5; i++) {
            if (player.get(i) != null) {
                playerGrid[i] = new String[] { // ✅ Correct syntax
                        "┌───────────────────┐",
                        "│" + Utils.center(player.get(i).getRace().getName() + " " + player.get(i).getRole().getName(),
                                19) + "│",
                        "├───────────────────┤",
        ...
        String[][] fieldGrid = {
                { " ", " ", " ", " ", "➤", Utils.center(turnOrder.get(0), 21 + 9),
                        Utils.center(turnOrder.get(1), 23 + 9), Utils.center(turnOrder.get(2), 21 + 9), " ", " ", " ",
                        " ", " " },
                { " ", "┌", "─", "─", "─", "─", "─", "─", "─", "─", "─", "┐", " " },
